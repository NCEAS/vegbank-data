...
plants <- plots_2 %>% 
  left_join(plots_1, by = "SurveyID",
            suffix = c("", "_small")) %>% 
  mutate(across(ends_with("_small"),
                ~coalesce(get(sub("_small", "", cur_column())), .))) %>% 
  select(-ends_with("_small"))
...
unmapped_code <- mapping_values %>% 
  filter(is.na(vb_pc_code2) & !is.na(vb_pc_code)) %>% 
  select(CodeSpecies, authorPlantName, vb_pc_code, vb_pc_code2)

# 6,355 plants didn't get matched
