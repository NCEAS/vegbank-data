...
This document shows rows that had to be dropped prior to ingest, and the reason that they were removed.

Below is a summary of all dropped rows:
...
strat_defs <- read_lt("strataDefinitionsLT.csv")
...
required_cols <- list(
  ...
  strat_defs = c("user_ob_code", "user_sr_code", "vb_sy_code"),
  strat    = c("user_ob_code", "user_sr_code", "author_plant_name", "user_tm_code"),
...
proj_codes <- unique(projects$user_pj_code)
...
a2 <- filter_with_audit(
  ...
  "plots: ProjectCode not found in projectLT"
)
plots_semi <- a2$kept
audit[["plots_missing_project_link"]] <- a2$dropped

plot_obs_codes <- unique(plots_semi$user_ob_code)

# Disturbance
a3 <- filter_with_audit(
  dist,
  user_ob_code %in% plot_obs_codes,
  "disturbance: disturbance SurveyID not found in filtered plotsLT"
)
dist_semi <- a3$kept
...
a4 <- filter_with_audit(
  comm,
  user_ob_code %in% plot_obs_codes,
  "community: SurveyID not found in filtered plotsLT"
)
comm_step1 <- a4$kept
...
a6 <- filter_with_audit(
  strat_defs, 
  user_ob_code %in% plot_obs_codes, 
  "strataDefinitions: user_ob_code not found in filtered plotsLT" ) 
strat_defs_step1 <- a6$kept 
audit[["strataDefinitions_missing_plot_link"]] <- a6$dropped 

a7 <- filter_with_audit( 
  strat_defs_step1, 
  !is.na(vb_sy_code), 
  "strataDefinitions: missing vb_sy_code (no strata mapping)" ) 
strat_defs_semi <- a7$kept 
audit[["strataDefinitions_missing_vb_sy_code"]] <- a7$dropped 

strata_sr_codes <- strat_defs_semi %>% 
  distinct() %>% 
  pull(user_sr_code)
...
a8 <- filter_with_audit(
  strat,
  user_ob_code %in% plot_obs_codes,
  "strataCover: SurveyID not found in filtered plotsLT"
)
strat_step1 <- a8$kept
...
tax_step1 <- a11$kept
audit[["taxon_missing_required_vb_pc_codes"]] <- a11$dropped

a11_1 <- filter_with_audit(
  tax,
  !is.na(vb_ro_code),
  "taxon: missing vb_ro_code"
)
  
tax_step1_1 <- a11_1$kept
audit[["taxon_missing_required_vb_pc_codes"]] <- a11_1$dropped
...
steps <- c(
  if (nrow(a2$dropped) > 0) "plots: missing project link",
  "disturbance: missing plot link",
  "community: missing plot link",
  "community: missing vb_cc_code",
  "strataDefinitions: missing plot link",
  "strataDefinitions: missing vb_sy_code",
...
drops <- c(
  if (nrow(a2$dropped) > 0) nrow(a2$dropped),
  nrow(a3$dropped),
  nrow(a4$dropped),
  nrow(a5$dropped),
  nrow(a6$dropped),
...
summary_tbl <- tibble(step = steps, dropped_rows = drops)
```
...
```{r}
if (nrow(audit[["plots_missing_project_link"]]) == 0) {
  cli::cli_alert_success("No plots dropped for missing project link.")
} else {
  x <- audit[["plots_missing_project_link"]] %>%
    select(any_of(c("user_ob_code", "user_pl_code", "user_pj_code", "drop_reason"))) %>%
    rename(SurveyID = user_ob_code, StandID = user_pl_code, ProjectCode = user_pj_code) %>%
    head(500)
  ...
  x %>%
    downloadthis::download_this(
      path = "plots_missing_project_link.csv",
      output_name = "plots_missing_project_link",
...
x <- audit[["disturbance_missing_plot_link"]] %>%
  select(any_of(c("user_ob_code", "type", "intensity", "drop_reason"))) %>%
  rename(SurveyID = user_ob_code) %>% 
  head(500)
DT::datatable(
    x,
    rownames = FALSE,
...
x %>% 
downloadthis::download_this(
  path = "disturbance_missing_plot_link.csv",
  output_name = "disturbance_missing_plot_link",
  button_label = "Download",
  button_type = "primary",
  has_icon = TRUE,
  icon = "fa fa-download")
...
# community classification dropped: not linked to a kept plot obs
```{r}
x <- audit[["community_missing_plot_link"]] %>%
  select(any_of(c("user_ob_code", "drop_reason"))) %>%
  rename(SurveyID = user_ob_code) %>% 
  head(500)
DT::datatable(
    x,
...
x %>% 
downloadthis::download_this(
  path = "community_missing_plot_link.csv",
  output_name = "community_missing_plot_link",
  button_label = "Download",
  button_type = "primary",
  has_icon = TRUE,
  icon = "fa fa-download")
...
x %>% 
downloadthis::download_this(
  path = "community_missing_vb_cc_code.csv",
  output_name = "community_missing_vb_cc_code",
  button_label = "Download",
  button_type = "primary",
  has_icon = TRUE,
  icon = "fa fa-download")
...
# strataCover (plants): not linked to a kept plot obs
```{r}
x <- audit[["strataCover_missing_plot_link"]] %>%
  select(any_of(c("user_ob_code", "author_plant_name", "drop_reason"))) %>%
  rename(SurveyID = user_ob_code) %>% 
  head(500)
DT::datatable(
    x,
...
x %>% 
downloadthis::download_this(
  path = "strataCover_missing_plot_link.csv",
  output_name = "strataCover_missing_plot_link.csv",
  button_label = "Download",
  button_type = "primary",
  has_icon = TRUE,
  icon = "fa fa-download")
...
# strataCover (plants): missing author_plant_name
```{r}
...
x %>% 
downloadthis::download_this(
  path = "strataCover_missing_author_plant_name.csv",
  output_name = "strataCover_missing_author_plant_name",
  button_label = "Download",
  button_type = "primary",
  has_icon = TRUE,
  icon = "fa fa-download")
...
cols <- c("user_to_code", "vb_pc_code", "vb_ro_code", "drop_reason")
...
x <- audit[["taxon_missing_strataCover_link"]] %>%
  select(any_of(c("user_to_code", "vb_pc_code", "vb_ro_code", "drop_reason"))) %>%
  head(500)
DT::datatable(
    x,
    rownames = FALSE,
...
x %>% 
downloadthis::download_this(
  path = "taxon_missing_strataCover_link.csv",
  output_name = "taxon_missing_strataCover_link",
  button_label = "Download",
  button_type = "primary",
  has_icon = TRUE,
  icon = "fa fa-download")
...
# strata definitions dropped: missing plot link
```{r}
x <- audit[["strataDefinitions_missing_plot_link"]] %>%
  select(any_of(c("user_ob_code", "user_sr_code", "vb_sy_code", "drop_reason"))) %>%
  head(500)
DT::datatable(
    x,
    rownames = FALSE,
...
# strata definitions dropped: missing vb_sy_code
```{r}
x <- audit[["strataDefinitions_missing_vb_sy_code"]] %>%
  select(any_of(c("user_ob_code", "user_sr_code", "vb_sy_code", "drop_reason"))) %>%
  head(500)
DT::datatable(
    x,
