...
out_dir <- here('data/loader-tables')
...
role_lookup_path <- '../data/lookup-tables/cdfw-roles-2026-03-18.csv'

role_lookup <- read_csv(role_lookup_path, progress = FALSE, show_col_types = FALSE)
...
```{r}
#| classes: styled-output
plots_loader(in_dir, out_dir, renew_cache)
```
...
Plot shapes (square, rectangle) are inferred from available dimension data when not explicitly recorded.
...
```{r}
#| classes: styled-output
community_loader(in_dir, out_dir, renew_cache)
```
...
```{r, echo = FALSE, warning = FALSE, message = FALSE}
method_lookup <- read_csv('../data/lookup-tables/classification-methods-20260318.csv', progress = FALSE, show_col_types = FALSE) %>% 
...
```{r}
#| classes: styled-output
stratacover_taxon_loader(in_dir, out_dir, renew_cache)
```
...
```{r}
method_lookup <- read_csv('../data/lookup-tables/CDFW-strata-method-20260304.csv', progress = FALSE, show_col_types = FALSE) %>% 
